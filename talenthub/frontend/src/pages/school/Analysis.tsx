    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <Card>
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="font-semibold text-slate-900">Bản đồ năng khiếu toàn trường</h2>
          </div>
          <svg viewBox={`0 0 ${SIZE} ${SIZE}`} className="w-full max-w-[340px] mx-auto">
            ...
            {data.skill_map.map((s, i) => {
              ...
              const [lx, ly] = polar(i, n, RADIUS + 26);
              return (
                <g key={s.code}>
                  <line x1={CENTER} y1={CENTER} x2={x} y2={y} stroke="#e2e8f0" strokeWidth="1" />
                  <text x={lx} y={ly - 2} textAnchor="middle" fontSize="10" fill="#64748b">{s.name}</text>
                  <text x={lx} y={ly + 11} textAnchor="middle" fontSize="12" fontWeight="bold" fill="#db2777">{s.avg_score}</text>
                </g>
              );
            })}
            <polygon points={valuePts} fill="rgba(236, 72, 153, 0.25)" stroke="#ec4899" strokeWidth="2" />
          </svg>
          <p className="text-xs text-slate-400 text-center mt-1">Điểm trung bình thang 100 từ dữ liệu kỹ năng học sinh.</p>
        </Card>

        <Card>
          <h2 className="font-semibold text-slate-900 mb-4">Bảng xếp hạng khối</h2>
            ...
            {data.grade_ranking.map((g, i) => (
              ...
              >
                ...
                <div className="flex-1">
                  <div className="font-semibold text-slate-800 text-sm">Khối {g.grade}</div>
                  <div className="text-xs text-slate-400">{g.hours}h hoạt động · {g.count} học sinh</div>
                </div>
                <div className="text-right">
                  <div className="text-lg font-bold text-pink-600">{g.avg_score}</div>
          ...
          <div className="mt-4 text-right">
            <Link to="/school/classes" className="text-sm text-blue-600 hover:underline">
      ...
      <Card>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold text-slate-900">Top học sinh nổi bật</h2>
          <span className="text-xs text-slate-400 ml-auto">Bấm để xem Talent Passport</span>
        </div>
            ...
            <tbody>
              {data.top_students.map((s, i) => (
                <tr key={s.id} className="border-b border-slate-100">
                  ...
                  <td className="py-2.5 text-center font-bold text-amber-600">{s.talent_score}</td>
                  <td className="py-2.5 text-center">{s.hours}h</td>
                  <td className="py-2.5 text-right">
                    <Link to={`/passport/${s.id}`} className="text-xs text-blue-600 hover:underline">
                      Xem passport →
                    </Link>
                  </td>
                </tr>
